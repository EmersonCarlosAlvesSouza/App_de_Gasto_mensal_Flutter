...
""");
  ...
  static List<GastoMensal> _toList(List<Map<String, dynamic>> result){final List<GastoMensal> gastos = [];
  ...
  static Future<int> alterar(GastoMensal gastoMensal) async {
    ...
    try {
      Database db = await instance.database;
      result = await db.update(table,Map.from(gastoMensal.getGastoMensal()), where: "$_id = ?", whereArgs:[gastoMensal.id]);
    } on Exception catch (e) {
      return 0;
    }
    return result;
